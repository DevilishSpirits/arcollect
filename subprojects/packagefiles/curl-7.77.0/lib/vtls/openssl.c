/* Patched openssl.c for Arcollect
 *
 * This edited file can be compiled in a Meson Unity build.
 * See → https://mesonbuild.com/Unity-builds.html ←
 *
 * protocols array has been renamed to openssl_c_protocols.
 */
/***************************************************************************
 *                                  _   _ ____  _
 *  Project                     ___| | | |  _ \| |
                                   ...
                                   struct connectdata *conn, int sockindex)
{
  ...
  */
  ...
  if(conn->bits.tls_enable_alpn) {
    int cur = 0;
    unsigned char openssl_c_protocols[128];
    ...
    if(data->state.httpwant >= CURL_HTTP_VERSION_2
      ...
      openssl_c_protocols[cur++] = ALPN_H2_LENGTH;

      memcpy(&openssl_c_protocols[cur], ALPN_H2, ALPN_H2_LENGTH);
      cur += ALPN_H2_LENGTH;
      infof(data, "ALPN, offering %s\n", ALPN_H2);
    }
#endif

    openssl_c_protocols[cur++] = ALPN_HTTP_1_1_LENGTH;
    memcpy(&openssl_c_protocols[cur], ALPN_HTTP_1_1, ALPN_HTTP_1_1_LENGTH);
     ...
     * format
     */
    if(SSL_CTX_set_alpn_protos(backend->ctx, openssl_c_protocols, cur)) {
