...
static const char mime_c_base64[] =
                                ...
                                curl_mimepart *part)
{
  ...
  while(st->bufbeg < st->bufend) {
    ...
    i = (i << 8) | (st->buf[st->bufbeg++] & 0xFF);
    *ptr++ = mime_c_base64[(i >> 18) & 0x3F];
    *ptr++ = mime_c_base64[(i >> 12) & 0x3F];
    *ptr++ = mime_c_base64[(i >> 6) & 0x3F];
    *ptr++ = mime_c_base64[i & 0x3F];
    cursize += 4;
    st->pos += 4;
    size -= 4;
  }
  ...
  if(ateof) {
    if(size < 4) {
      ...
      case 1:
        ...
        ptr[0] = mime_c_base64[(i >> 18) & 0x3F];
        ptr[1] = mime_c_base64[(i >> 12) & 0x3F];
        if(++st->bufbeg != st->bufend) {
          ptr[2] = mime_c_base64[(i >> 6) & 0x3F];
          st->bufbeg++;
        }
        cursize += 4;
        st->pos += 4;
