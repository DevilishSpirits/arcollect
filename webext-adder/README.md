...
**Note!** This protocol is meant for Arcollect internal workings and there is no stability promise. Though that is the best way to add new artworks in Arcollect if you want to do so.
...
* The `"transaction_id"` and whole communication is managed for you, do not set it.
* The `"dns_prefill"` is automatically generated by scanning your JSON, do not set it.
...
It's a JSON based protocol prefixed with a 32 bits integer that contains the JSON length in native byte-order as defined by the *nativeMessaging* standard. Given a JSON you can generate a valid payload you can pipe in the standard input of the webext-adder using [make_test_payload.py](tests/make_test_payload.py).
The JSON is fully parsed before performing a transaction on the database so items order in the JSON is not important.
Note that the `webext-adder` isn't meant to scale on very large dataset such as a GB database dump.
Each queries are ACID and have no timeout.
...
```json
{
	...
	"com_tag_links": [{
		"comic": "https://example.net/art/123456/",
		"tag": 2134,
	}]
}
```
...
The `art_acc_links` array contain links with artworks and profiles :
...
The `art_tag_links` array contain links with artworks and tags :
* `artwork` is the artwork `"source"` to link.
* `tag` is the tag `"id"`.

The `com_*_links` arrays are like `art_*_links` but instead of `"artwork"`, you set a `"comic"` field set to it's `"id"`.
