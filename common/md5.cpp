...
void MD5_CTX::Update(const void *data, std::size_t size)
{
	std::uint_fast32_t saved_lo;
	...
	if (size >= 64) {
		data = body(*this, data, size & ~(std::size_t)0x3f);
		size &= 0x3f;
	}
...
MD5_CTX::DIGEST MD5_CTX::Final(void)
{
	std::size_t used, available;
